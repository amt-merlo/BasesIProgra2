...
---------------------------PARA PARSEAR PERSONAS---------------------------
	...
	ADD CONSTRAINT modificar_fecha2
	DEFAULT @Fecha FOR FechaConstitucion
	...
	SELECT TipoCuentaId, NumeroCuenta, ValorDocID, FechaConstitucion, Saldo FROM #TablaCuentas
	...
	SELECT @Movimientos =  @XML.query('Operaciones/FechaOperacion[@Fecha=sql:variable("@Fecha")]//Movimientos')
	 ...
	 ------------------------------
	 --Tabla variable
	 CREATE TABLE #TablaMovimientos(Tipo int, CodigoCuenta int, Monto float, Descripcion nchar(30), Fecha date, Visible bit, EstadodeCuenta int)
	 ...
	 ALTER TABLE #TablaMovimientos
	 ADD CONSTRAINT modificar_fechaMovimiento
	 DEFAULT @Fecha FOR Fecha

	 ALTER TABLE #TablaMovimientos 
	 ADD CONSTRAINT modificar_visible
	...
	SELECT @lo = COUNT(*) from #Movimientos
	...
	WHILE @lo <= @hi
		...
		IF @cantEstadosCuenta > 0
			--TOMAMOS EL ID DEL ESTADO DE CUENTA
			DECLARE @EstadodeCuentaID as int
			Select @EstadodeCuentaID = ID FROM dbo.EstadodeCuenta WHERE @Fecha BETWEEN FechaInicio AND FechaFin

			--Asignamos el estado de cuenta que encontró 
			ALTER TABLE #TablaMovimientos 
			ADD CONSTRAINT modificar_estadocuenta
			DEFAULT @EstadodeCuentaID FOR EstadodeCuenta

		ELSE
			--Tomamos los datos del ultimo estado de cuenta registrado
			DECLARE @
			--Creamos un nuevo estado de cuenta
