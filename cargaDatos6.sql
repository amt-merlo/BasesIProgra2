...
--Tabla variable
	...
	--Tabla variable
	 ...
	 INSERT INTO @EsAdministrador (esAdmin)
		SELECT 	xmlData.A.value('.', 'date') AS EsAdmin 
	...
	INSERT INTO dbo.Usuario(ValorDocumentoIdentidadID, Username, Pass, EsAdministrador)
	SELECT ValorDocumentoIdentidad, Usuario, Pass, EsAdministrador from @TablaUsuarios

	---------------------------PARA PARSEAR USUARIOPUEDEVER---------------------------
	...
	BEGIN
		...
		BEGIN
			...
			SELECT @totalEstadosdeCuenta = COUNT(*) FROM dbo.EstadodeCuenta
			...
			UPDATE dbo.CuentadeAhorro
			SET Saldo = Saldo + (Saldo*@TasaInteres)/100
			WHERE NumerodeCuenta = @NumCuentaEstado
